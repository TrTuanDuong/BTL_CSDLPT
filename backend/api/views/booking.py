...
from ..serializers.booking import (
    BookingSerializer, BookingCreateSerializer, BookingDetailSerializer, TicketSerializer
)
...
class BookingViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Booking.objects.filter(user=self.request.user).select_related(
            'showtime__movie', 'showtime__auditorium'
    ...
    def cancel(self, request, pk=None):
        ...
        if booking.status not in ['reserved']:
            return Response({
                'error': 'Chỉ có thể hủy booking đang tạm giữ ghế'
...
class TicketViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    ordering = ['-booking__booking_time']
    
    def get_queryset(self):
        # User chỉ xem được ticket của mình
        return Ticket.objects.filter(
            booking__user=self.request.user
        ).select_related('seat', 'showtime__movie', 'booking')
